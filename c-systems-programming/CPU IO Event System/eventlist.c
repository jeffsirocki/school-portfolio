...
//   isEmpty, add_event, delete, get_nextevent, print_eventlist
...
void add_event( ListNodePtr *nPtr, Event event, int sort ) {
  ...
  if( newPtr != NULL ) {
    ...
    while( currentPtr != NULL && value > currentPtr->event.sort ) {
      previousPtr = currentPtr;
      currentPtr = currentPtr->nextPtr;
    }

    // loop to find correct location if a tie
    while( currentPtr != NULL && value == currentPtr->event.sort && event.process_id > currentPtr->event.process_id ) {
      previousPtr = currentPtr;
      currentPtr = currentPtr->nextPtr;
    }
